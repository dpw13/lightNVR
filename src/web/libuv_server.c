...
static http_server_handle_t libuv_server_init_internal(const http_server_config_t *config,
    ...
    log_info("libuv_server_init: Server initialized on %s:%d", inet_ntoa(config->bind_ip), config->port);
...
int libuv_server_start(http_server_handle_t handle) {
    ...
    addr.sin_addr = server->config.bind_ip;
    ...
    server->running = true;
    log_info("libuv_server_start: Listening on %s:%d", inet_ntoa(server->config.bind_ip), server->config.port);
